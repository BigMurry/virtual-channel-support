...
module.exports = async (channel) => {
  ...
  if(phone1) {
  	twilio.messages.create(
    {
      to: phone1,
      from: '+12157039437',
      body: '[For ' + channel.address1 + '] Status update for channel [channelID: ' + channelID + ']. [Status: ' + channel.status
    },
    (err, message) => {
    	error1 = err;
    }
  ...
  if(phone2) {
  	twilio.messages.create(
    {
      to: phone2,
      from: '+12157039437',
      body: '[For ' + channel.address2 + '] Status update for channel [channelID: ' + channelID + ']. [Status: ' + channel.status
    },
    (err, message) => {
    	error2 = err;
    }
